...
> **Hạ tầng triển khai**: Google Colab Pro (GPU A100 SXM4, 300 Compute Units) kết hợp Google Drive 5TB nhóm (Phương án A - Shortcut 0 byte).  
...
## IV. KIẾN TRÚC VẬN HÀNH HẠ TẦNG (GOOGLE COLAB PRO & DRIVE 5TB)

Dự án áp dụng cơ chế kết nối **Phương Án A (Shortcut Protocol)** nhằm tối ưu chi phí và chống sập I/O trên Google Colab Pro:

```mermaid
sequenceDiagram
    autonumber
    actor Team as Nhóm Nghiên Cứu
    participant Hub as Tài Khoản Chính (Drive 5TB)
    participant Colab as Tài Khoản Colab Pro (A100 + SSD)
    participant Model as FatFormer-XLA Framework

    Team->>Hub: Tạo thư mục 'FatFormer_Hub' lưu Checkpoints & Datasets (.tar)
    Team->>Colab: Chia sẻ 'FatFormer_Hub' -> Bấm 'Thêm lối tắt vào Drive' (0 Byte)
    Colab->>Colab: Mount Google Drive tại /content/drive/MyDrive/FatFormer_Hub
    Note over Colab: Quy Tắc I/O Chống Sập
    Colab->>Colab: Copy file .tar từ Drive sang SSD NVMe (/content/)
    Colab->>Colab: Giải nén vào /content/dataset_local/ (Tốc độ tăng 10x)
    ...
    Model->>Colab: CheckpointManager lưu checkpoint_epoch_X.pth cục bộ
    Model->>Hub: Tự động copy đồng bộ sang Drive 5TB (Chống ngắt phiên làm việc)
```
